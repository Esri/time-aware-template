...
define([
  ...
  return declare(null, {
    ...
    _updatePlayButton: function (add, remove) {
      ...
      var buttonTitle = add === "esri-icon-play" ? this.config.i18n.time.play : this.config.i18n.time.pause;
      domAttr.set(play, "title", buttonTitle);
    },
    _displayTime: function () {
      ...
      if (this.config.durationTime && this.config.durationPeriod && this.config.tickTime && this.config.tickPeriod) {
        ...
        if (this.config.futureDuration) {
          customStart = moment();
          customEnd = moment().add(this.config.durationTime, this.config.durationPeriod);
        } else {
          customEnd = moment(); //.format("MMMM Do YYYY , h:mm:ss a");
          customStart = moment().subtract(this.config.durationTime, this.config.durationPeriod); //.format("MMMM Do YYYY , h:mm:ss a");            
        }
      ...
      if (timeProperties && timeExtent) {
        ...
        // Add speed button if enabled
        if (this.config.timespeed) {
          require(["dijit/form/DropDownButton", "dijit/DropDownMenu", "dijit/MenuItem"], lang.hitch(this, function (DropDownButton, DropDownMenu, MenuItem) {
            var speedContainer = dom.byId("timeSpeedContainer");
            domClass.remove(speedContainer, "hide");
            // Add dropdown menu with rates 
            var rates = [0.25, 0.5, 1, 1.5, 2];
            var rateMenu = new DropDownMenu({
              style: "display:none;"
            });
        ...
        // modify thumb position if current time extent is saved in webmap 
        if (timeProperties.currentTimeExtent && timeProperties.currentTimeExtent.length && timeProperties.currentTimeExtent.length > 0) {
          var startTimeDate = null,
            endTimeDate = null;
          var startPos, endPos;
          startTimeDate = timeProperties.currentTimeExtent[0];
          if (timeProperties.currentTimeExtent.length > 1) {
            endTimeDate = timeProperties.currentTimeExtent[1];
          }
          timeSlider.timeStops.forEach(function (stop, i) {
            if (stop.getTime() === startTimeDate) {
              startPos = i;
            } else if (stop.getTime() === endTimeDate) {
              endPos = i;
            }
          });
          if (startPos) {
        ...
        if (this.config.autoplay) {
          timeSlider.play();
          this._updatePlayButton("esri-icon-pause", "esri-icon-play");
        }
        ...
        on(dom.byId("playSlider"), "click", lang.hitch(this, function () {
          var play = domClass.contains("playSlider", "esri-icon-play");

          var removeClass = null,
            addClass = null;
          if (play) { //Switch to the pause icon and press play
            removeClass = "esri-icon-play";
            addClass = "esri-icon-pause";
            timeSlider.play();
          } else { //Switch to the play icon and press pause
            removeClass = "esri-icon-pause";
            addClass = "esri-icon-play";
            timeSlider.pause();
          }
        ...
        on(timeSlider, "time-extent-change", lang.hitch(this, function (e) {
          ...
          if (val == slider.maximum && timeSlider.loop !== true) {
            ...
            this._updatePlayButton("esri-icon-play", "esri-icon-pause");
    ...
    _createWebMap: function (itemInfo, params) {
      ...
      if (this.config.disableNav) {
        this.config.zoomslider = false;
      }
